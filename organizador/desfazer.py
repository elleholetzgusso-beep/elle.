...
def desfazer_operacao(operacao: dict, *, simular: bool = False) -> ResultadoDesfazer:
    ...
    for movimento in reversed(operacao.get("movimentos", [])):
        ...
        if not destino_atual.exists():
            resultado.problemas.append(f"no encontrado: {destino_atual}")
            continue
        ...
        if alvo != origem:
            resultado.problemas.append(
                f"'{origem}' ya existe; restaurado como '{alvo.name}'"
            )
        ...
        if not simular:
            try:
                ...
                resultado.problemas.append(f"fallo al restaurar {destino_atual}: {erro}")
                continue
    ...
    for pasta in criadas:
        ...
        if any(pasta.iterdir()):
            resultado.problemas.append(f"carpeta no está vacía, se mantiene: {pasta}")
            continue
        if not simular:
            try:
                pasta.rmdir()
            except OSError as erro:
                resultado.problemas.append(f"fallo al eliminar {pasta}: {erro}")
                continue
    ...
    for pasta in removidas:
        ...
        if not simular:
            try:
                ...
                resultado.problemas.append(f"fallo al recrear {pasta}: {erro}")
                continue
