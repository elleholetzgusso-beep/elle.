...
def resumir(operacao: dict) -> str:
    ...
    partes = []
    if quantidade_mov:
        partes.append(f"{quantidade_mov} archivo(s) movido(s)")
    if quantidade_criadas:
        partes.append(f"{quantidade_criadas} carpeta(s) creada(s)")
    if quantidade_removidas:
        partes.append(f"{quantidade_removidas} carpeta(s) eliminada(s)")
    detalhe = ", ".join(partes) if partes else "sin cambios"
