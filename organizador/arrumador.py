...
CRITERIOS = ("tipo", "extensao", "data", "alfabetico", "documento")
...
def _subpasta(caminho: Path, criterio: str, indice: dict[str, str], formato_data: str | None) -> str:
    if criterio == "tipo":
    ...
    raise ErroDeOrganizacao(
        f"Criterio '{criterio}' desconhecido. Use: {', '.join(CRITERIOS)}."
    )
...
def organizar(
    ...
    *,
    ...
    if not base.is_dir():
        raise ErroDeOrganizacao(f"A pasta '{base}' nao existe.")
    ...
    for arquivo in _listar_arquivos(base, recursivo, incluir_ocultos):
        if any(fnmatch(arquivo.name, padrao) for padrao in ignorar):
            resultado.ignorados.append((arquivo, "ignorado por padrao"))
            continue

        try:
            nome_subpasta = _subpasta(arquivo, criterio, indice, formato_data)
        except OSError as erro:
            resultado.ignorados.append((arquivo, f"nao foi possivel ler: {erro}"))
            continue

        destino_pasta = base.joinpath(*nome_subpasta.split("/"))
        if arquivo.parent == destino_pasta:
            resultado.ignorados.append((arquivo, "ja esta organizado"))
            continue
        ...
        if not simular:
            ...
            try:
                ...
                resultado.ignorados.append((arquivo, f"falha ao mover: {erro}"))
                continue
        else:
