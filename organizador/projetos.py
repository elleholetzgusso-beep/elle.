...
def limpar_nome(texto: str) -> str:
    ...
    if not limpo:
        raise ErroDeProjeto("O nome informado ficou vazio depois da limpeza.")
    return limpo
...
def nome_projeto(ano: int | str, codigo: str, nome: str) -> str:
    ...
    if not re.fullmatch(r"\d{4}", ano_texto):
        raise ErroDeProjeto(f"Ano invalido: '{ano}'. Use quatro digitos, ex.: 2026.")
    codigo_limpo = limpar_nome(str(codigo)).replace(" ", "")
    if not codigo_limpo:
        raise ErroDeProjeto("Informe o codigo do projeto, ex.: 16883.")
...
def pasta_recebida(projeto: str | Path) -> Path:
    ...
    if not caminho.is_dir():
        raise ErroDeProjeto(f"A pasta '{caminho}' nao existe.")
    ...
    *,
    ...
    if isinstance(data_envio, str):
        ...
        for formato in ("%Y%m%d", "%Y-%m-%d", "%d/%m/%Y"):
            ...
            raise ErroDeProjeto(
                f"Data invalida: '{data_envio}'. Use AAAAMMDD, AAAA-MM-DD ou DD/MM/AAAA."
            )
    else:
    ...
    if numero is None:
        ...
        raise ErroDeProjeto("O numero do envio deve ser 1 ou maior.")

    pasta = destino_base / nome_envio(numero, quando, observacao)
    if pasta.exists():
        raise ErroDeProjeto(f"A pasta '{pasta}' ja existe.")
    if not simular:
