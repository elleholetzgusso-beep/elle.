    ...
    for pasta in reversed(faltando):
        if not simular:
            try:
                ...
                raise ErroDeCriacao(f"Nao foi possivel criar '{pasta}': {erro}") from erro
...
def criar_estrutura(
    ...
    *,
    ...
    if raiz.exists() and not raiz.is_dir():
        raise ErroDeCriacao(f"O destino '{raiz}' existe e nao e uma pasta.")
    ...
    for bruto in caminhos:
        ...
        if completo.exists():
            if not completo.is_dir():
                raise ErroDeCriacao(
                    f"Ja existe um arquivo com o nome '{completo}'; renomeie antes de continuar."
                )
