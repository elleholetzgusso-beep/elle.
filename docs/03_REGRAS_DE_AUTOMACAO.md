...
| **Nome do ficheiro final** | PE/05 | `pipeline.Config.lpa`/`.anejo` | Na janela, o `LPA.xlsm` genérico vira o nome PE/05 (`EXC.../002/LPA/05.xlsm`) assim que a `portada.referencia` é válida — `nomenclatura.nome_ficheiro()`. Sem referência válida, cai no genérico; não se inventa um código. |
| **Destaque do que é novo** | — (convenção da casa, não normativa) | `novidade.py`, `fill --anterior` | Com um LPA anterior para comparar, o que for novo sai a azul (`#0070C0`): puntos inteiros, diálogo acrescentado, documentos/envíos, a revisão. Comparação real contra um ficheiro — nunca "tudo o que não é do template", que confundiria conteúdo trazido do `extract` com conteúdo desta revisão. Sem `--anterior`, nada se destaca. |
...
| `novidade.py` | O que é novo entre o LPA anterior e o projeto atual, para o `filler` pintar a azul. |
